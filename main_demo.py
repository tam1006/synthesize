    ...
    def connect_images(self, x_num=6, y_num=2, progress=91):
        ...
        for i in range(0,x_num,2):
            ...
            if y_num == 1:
                ...
                second_image = CN.auto_img_concat_v(compare_h=40, compare_w=1900, tri_h=150, tri_w=10, search_start=300, search_h=300, limit_h_init=699, limit_w_init=14)
        ...
        # Phase 3
        print('########  Phase 3 ######')
        for i in range(0,x_num//2,2):
            if self.flags.suspend_flag is True:
                    return 0
            CN.update_images(f"temp1/image{i+2}.png",f"temp1/image{i}.png")
            if y_num == 1:
                # CN.img1 = cv2.rotate(CN.img1, cv2.ROTATE_90_COUNTERCLOCKWISE)
                # CN.img2 = cv2.rotate(CN.img2, cv2.ROTATE_90_COUNTERCLOCKWISE)
                second_image = CN.auto_img_concat_v(compare_h=40, compare_w=1220, tri_h=150, tri_w=10, search_start=900, search_h=300, limit_h_init=699, limit_w_init=14)
                small_w = second_image.shape[1]
                small_h = second_image.shape[0]     
            else:
                second_image = CN.auto_img_concat_v(compare_h=40, compare_w=2600, tri_h=100, tri_w=10, search_start=550, search_h=300, limit_h_init=649, limit_w_init=20)
            second_image = cv2.rotate(second_image, cv2.ROTATE_180)
            print("高さ:", second_image.shape[0])
            cv2.imwrite(f"temp2/image{i}.png",second_image)
            progress += res_percent_ratio
            send_data = {
                "type": "progress",
                "data": progress
        ...
        if x_num == 4:
            final_image = cv2.imread("temp2/image0.png")
        ...
        # Phase 4
        print('###### Phase 4 ######')
        CN.update_images("temp2/image0.png","temp2/image2.png")
        if self.flags.suspend_flag is True:
            return 0
        if y_num == 1:
        ...
        cv2.imwrite(f"temp2/semi.png",semi_image)

        print("高さ:",semi_image.shape[0])

        if x_num == 4:
            final_image = cv2.imread("temp2/semi.png")
            final_image = cv2.rotate(final_image, cv2.ROTATE_90_CLOCKWISE)
            # cv2.imwrite("final.png", final_image)
            return final_image
        ...
        # Phase 5
        print('###### Phase 5 ######')
        CN.update_images("temp2/semi.png","temp1/image4.png")
        if self.flags.suspend_flag is True:
            return 0
        if y_num == 1:
